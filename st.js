...
function getTotalClickThreshold() {
  const script = document.querySelector('script[src$="impressions-clicks.js"]');
  return parseInt(script.getAttribute('data-total-click-threshold') || 10);
}

// Function to retrieve the reset interval from the script attribute
function getResetInterval() {
  const script = document.querySelector('script[src$="impressions-clicks.js"]');
  return parseInt(script.getAttribute('data-reset-interval') || 86400);
}
